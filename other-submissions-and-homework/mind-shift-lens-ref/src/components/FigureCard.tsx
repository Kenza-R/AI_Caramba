...
const ringColor = (intensity: Figure["shiftIntensity"]) => {
  switch (intensity) {
    case "stable": return "hsla(185, 100%, 50%, 0.6)";
    case "moderate": return "hsla(37, 90%, 55%, 0.7)";
    case "significant": return "hsla(0, 75%, 55%, 0.7)";
  }
};

const ringBorder = (intensity: Figure["shiftIntensity"]) => {
  switch (intensity) {
    case "stable": return "border-primary/60";
    case "moderate": return "border-amber/70";
    case "significant": return "border-destructive/70";
  }
};
    ...
    >
      ...
      <div className="relative mb-4">
        ...
        >
          ...
          />
          <div className={`w-24 h-24 rounded-full border-2 ${ringBorder(figure.shiftIntensity)} bg-muted flex items-center justify-center overflow-hidden relative z-10`}>
            <span className="font-display text-2xl font-bold text-foreground/80">{initials}</span>
          </div>
        </motion.div>
